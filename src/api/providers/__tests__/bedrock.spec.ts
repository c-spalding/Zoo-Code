...
vi.mock("@aws-sdk/client-bedrock", () => {
	...
	const seededProfileIds = [
		...
		// Claude Opus 4.8 - exercises the adaptive-thinking cross-region prefix test,
		// modelling AWS having published the regional profile so profile-gating applies it.
...
describe("AwsBedrockHandler", () => {
	...
	describe("Claude 4.7+ adaptive thinking (Opus 4.7 / Opus 4.8)", () => {
		...
		it("should send adaptive thinking with effort xhigh for Claude Opus 4.7 when reasoning is enabled", async () => {
			// xhigh is only emitted when the user explicitly opts in via reasoningEffort.
			// The budget-derived fallback never escalates to xhigh/max (see
			// mapReasoningBudgetToBedrockEffort), so the explicit setting is required here.
		...
		it("should send adaptive thinking with effort xhigh for Claude Opus 4.8 when reasoning is enabled", async () => {
			// As with 4.7, xhigh requires an explicit user-selected reasoningEffort;
			// the budget fallback only ever yields low/medium/high.
